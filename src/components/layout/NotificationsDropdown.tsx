...
import { useAuthStore, useCountryStore } from '@/store/useStore';
...
export default function NotificationsDropdown() {
  ...
  // When backend returns 403 (no permission / unverified email), stop polling
  ...
  const getIcon = (type: string) => {
    if (type.includes('Contact')) return <MessageSquare size={16} className="text-orange-500" />;
  ...
  const getLink = (notification: Notification): string => {
    ...
    if (raw) {
      // Convert any dashboard article URL → frontend article page
      const articleMatch = raw.match(/\/dashboard\/articles\/(?:edit\/)?(\d+)/);
      if (articleMatch) return `/${countryCode}/lesson/articles/${articleMatch[1]}`;

      // Convert any dashboard post URL → frontend post page
      const postMatch = raw.match(/\/dashboard\/posts\/(?:edit\/)?(\d+)/);
      if (postMatch) return `/${countryCode}/posts/${postMatch[1]}`;

      // Contact messages → messages page with contact tab active
      if (raw.includes('/dashboard/contact-messages')) return '/dashboard/messages?tab=contact';

      // Internal messages or other dashboard routes → use as-is
      if (raw.includes('/dashboard/messages')) return '/dashboard/messages';
      if (!raw.startsWith('/dashboard/')) return raw;
    }

    // Fallbacks by notification type
    const t = notification.type;
    if (t.includes('Contact')) return '/dashboard/messages?tab=contact';
    if (t.includes('Message')) return '/dashboard/messages';
    if (t.includes('Article')) return `/${countryCode}/lesson/articles`;
    if (t.includes('Post')) return `/${countryCode}/posts`;
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="fixed left-2 right-2 top-[68px] md:absolute md:left-0 md:right-auto md:top-auto md:mt-2 md:w-96 bg-card border border-border rounded-xl shadow-xl z-[999] animate-in fade-in slide-in-from-top-2 overflow-hidden ring-1 ring-black/5">
