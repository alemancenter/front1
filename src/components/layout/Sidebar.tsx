...
export default function Sidebar({ initialSettings }: SidebarProps) {
  ...
  const menuItems = useMemo(() => {
    const hasPermission = (permission?: string): boolean => {
      ...
      // Check aliases (direct and role-based)
      const aliases = permissionAliases[permission] || [];
      if (aliases.length === 0) return false;
      const hasAliasPermission = user.permissions?.some(p => aliases.includes(p.name)) ?? false;
      if (hasAliasPermission) return true;
      return user.roles?.some(role =>
        role.permissions?.some(p => aliases.includes(p.name))
      ) ?? false;
    };
    ...
    const items = [
      ...
      },
      {
        ...
        children: [
          ...
          { title: 'تذكيرات تحقق البريد', href: '/dashboard/settings/email-verification', permission: 'manage settings' },
          { title: 'البريد المرتد (Bounce)', href: '/dashboard/settings/email-bounce', permission: 'manage settings' },
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 right-4 z-50 p-2 bg-primary text-primary-foreground rounded-xl shadow-lg hover:bg-primary/90 transition-colors"
      >
      ...
      {/* Sidebar Container */}
      <aside
        className={`dashboard-sidebar fixed top-0 right-0 h-screen transition-all duration-300 ease-in-out z-50 flex flex-col
          ${isOpen ? 'w-72 translate-x-0' : 'w-72 translate-x-full lg:translate-x-0 lg:w-20'}
        `}
      >
        {/* Logo Section */}
        <div className="dashboard-sidebar-logo h-16 flex items-center justify-between px-4 shrink-0">
          <div className={`flex items-center gap-3 transition-opacity duration-200 ${!isOpen && !isMobile ? 'opacity-0 w-0 overflow-hidden' : 'opacity-100'}`}>
            {displaySiteLogo ? (
              ...
              <div className="w-10 h-10 bg-white/12 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-sm border border-white/10">
                {displaySiteName ? displaySiteName.charAt(0).toUpperCase() : 'R'}
              </div>
            )}
            <Link href="/" className="text-lg font-black text-white whitespace-nowrap hover:opacity-90 transition-opacity">
              {displaySiteName}
            </Link>
          </div>
          ...
          {!isMobile && (
            <button 
              onClick={handleToggle}
              className={`p-1.5 hover:bg-white/10 rounded-xl text-white/80 transition-all duration-200 ${!isOpen ? 'mx-auto' : ''}`}
            >
                      ...
                      return (
                        <Link
                          ...
                          className={`block py-2 px-3 text-sm rounded-lg transition-colors
                            ${isChildItemActive 
                              ? 'dashboard-submenu-item-active font-black' 
        ...
        {/* Footer Section */}
        <div className="dashboard-sidebar-footer p-4 backdrop-blur-sm shrink-0">
          <button 
            onClick={handleLogout}
            className={`flex items-center gap-3 w-full px-3 py-3 rounded-xl text-red-200 hover:bg-red-500/15 hover:text-white transition-colors
            ${(!isOpen && !isMobile) ? 'justify-center' : ''}
          `}>
            <LogOut size={22} />
            <span className={`whitespace-nowrap transition-all duration-300 ${(!isOpen && !isMobile) ? 'opacity-0 w-0 hidden' : 'opacity-100'}`}>
