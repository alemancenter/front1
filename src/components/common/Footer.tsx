...
export default function Footer({ initialSettings }: { initialSettings?: SettingsBag }) {
  ...
  const displaySiteName = initialSiteName || (siteName || '').toString().trim() || 'الإيمان التعليمي';
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12">
          ...
          <div className="lg:col-span-3">
            ...
            <ul className="space-y-3">
              ...
              <li>
                <Link href="/copyright" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors group">
                  <Shield className="w-4 h-4 group-hover:translate-x-[-2px] transition-transform" />
                  <span>حقوق الملكية</span>
                </Link>
              </li>
              <li>
                <Link href="/editorial-policy" className="flex items-center gap-2 text-muted-foreground hover:text-primary transition-colors group">
                  <FileText className="w-4 h-4 group-hover:translate-x-[-2px] transition-transform" />
                  <span>سياسة التحرير</span>
                </Link>
              </li>
            </ul>
          </div>

          {/* Trust Notice (col-lg-3) */}
          <div className="lg:col-span-3">
            <div className="bg-muted/30 rounded-2xl p-6 border border-border/50">
              <h3 className="font-bold text-foreground text-lg mb-3">منصة تعليمية موثوقة</h3>
              <p className="text-sm text-muted-foreground leading-7">
                نوفر محتوى وملفات تعليمية منظمة للاستخدام الشخصي والتعليمي، مع صفحات واضحة للخصوصية وحقوق الملكية والتواصل.
              </p>
              <div className="mt-5 space-y-2 text-sm font-bold text-muted-foreground">
                <Link href="/copyright" className="block rounded-xl border border-border bg-background px-4 py-3 transition hover:text-primary">طلبات حقوق الملكية والإزالة</Link>
                <Link href="/editorial-policy" className="block rounded-xl border border-border bg-background px-4 py-3 transition hover:text-primary">سياسة التحرير والمراجعة</Link>
              </div>
            </div>
          </div>
