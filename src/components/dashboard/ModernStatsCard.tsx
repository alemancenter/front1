...
export default function ModernStatsCard({
  ...
  if (isLoading) {
    return (
      <div className="dashboard-stat-card relative overflow-hidden rounded-2xl p-6 animate-pulse">
  ...
  return (
    <motion.div
      ...
      className="dashboard-stat-card relative overflow-hidden rounded-2xl p-6 transition-all duration-300 group cursor-pointer"
      whileHover={{ y: -5 }}
    >
      <div className="relative z-10">
        <div className="flex justify-between items-start mb-4">
          <div className={`p-3 rounded-2xl bg-white/75 dark:bg-white/10 backdrop-blur-md border border-border/60 shadow-inner ${color}`}>
            {icon}
          </div>
          {changeType === 'increase' ? (
            <div className="flex items-center gap-1 text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-500/15 px-2 py-1 rounded-lg text-xs font-medium">
              <ArrowUp size={14} />
              <span>+{change}%</span>
            </div>
          ) : (
            <div className="flex items-center gap-1 text-rose-700 dark:text-rose-300 bg-rose-100 dark:bg-rose-500/15 px-2 py-1 rounded-lg text-xs font-medium">
              <ArrowDown size={14} />
              <span>-{change}%</span>
            </div>
          )}
