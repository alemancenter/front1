...
async function normalizeAndPersistAuthResponse(responseData: unknown): Promise<AuthResponse> {
  ...
  return {
    ...
    token,
    user,
    verification_email_sent: raw.verification_email_sent ?? raw.data?.verification_email_sent,
  };
}
...
export const authService = {
  ...
  async checkEmail(email: string): Promise<{ available: boolean }> {
    const res = await apiClient.post<{ available: boolean }>(
      ...
      { email },
      { timeout: 5000, retries: 0, cache: 'no-store' },
    );
    return res.data as { available: boolean };
  },

  async preflightEmail(email: string): Promise<EmailPreflightResult> {
    const response = await apiClient.post<any>(
      API_ENDPOINTS.AUTH.EMAIL_PREFLIGHT,
      { email },
      { timeout: 8000, retries: 0, cache: 'no-store' },
    );
    return (response.data?.data ?? response.data) as EmailPreflightResult;
  },
  ...
  async changeUnverifiedEmail(email: string): Promise<{ user: User; verification_email_sent: boolean; message: string }> {
    const response = await apiClient.post<any>(
      API_ENDPOINTS.AUTH.CHANGE_EMAIL,
      { email },
      {
        cache: 'no-store',
      }
    ...
    return {
      message: raw?.message ?? '',
      user: (raw?.data?.user ?? raw?.user) as User,
      verification_email_sent: Boolean(raw?.data?.verification_email_sent ?? raw?.verification_email_sent),
    };
  },
