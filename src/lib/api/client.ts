...
class ApiClient {
  ...
  constructor() {
    ...
    if (!/^https?:\/\//i.test(this.baseUrl)) {
      this.baseUrl = process.env.NEXT_PUBLIC_API_URL || 'https://api.alemancenter.com/api';
    }
  ...
  private buildUrl(
    ...
    const rawBaseUrl = this.getCurrentBaseUrl().replace(/\/+$/, '');
    const isAbsoluteBaseUrl = /^https?:\/\//i.test(rawBaseUrl);

    /*
     * Important:
     * Browser API base can be relative, for example:
     *   /backend-api
     *
     * new URL('/backend-api/comments/1') is invalid without origin.
     * So we temporarily convert it to an absolute URL, then return
     * a clean relative URL for browser requests.
     */
    const origin =
      typeof window !== 'undefined'
        ? window.location.origin
        : process.env.NEXT_PUBLIC_SITE_URL || 'https://alemancenter.com';
    ...
    if (params) {
      ...
      const url = new URL(`${absoluteBaseUrl}${finalEndpoint}`);
    ...
    if (isAbsoluteBaseUrl) {
      return `${rawBaseUrl}${finalEndpoint}`;
    }
