...
// Parse composite content ID like 'jo:19' → { countryCode: 'jo', id: '19' }
function splitCompositeId(compositeId: string): { countryCode: string; id: string } {
  const idx = compositeId.indexOf(':');
  if (idx > 0) {
    return { countryCode: compositeId.slice(0, idx), id: compositeId.slice(idx + 1) };
  }
...
export default function ContentAuditPage() {
  ...
  const handleSelectFinding = async (finding: PolicyAuditFinding) => {
    ...
    try {
      ...
      const latest = await contentAuditService.getLatestDecision(finding.type, id, countryCode);
  ...
  const handleAnalyzeAI = async () => {
    ...
    try {
      ...
      setMessage(null);
      const { countryCode: auditCountry, id: auditId } = splitCompositeId(selectedFinding.id);
      ...
      let text: string;
      if (status === 409) {
        text = 'يوجد تحليل ذكاء اصطناعي قيد التنفيذ لهذا المحتوى. انتظر قليلًا ثم حدّث القرار.';
      } else if (status >= 500) {
        text = 'فشل محرك فحص المحتوى في الخادم. تم تسجيل السبب في سجلات API، ولم يتم تعديل المحتوى.';
      } else {
