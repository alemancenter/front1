...
import PaginationControls from "@/features/content-audit/components/PaginationControls";
import {
  riskLabels,
  riskOptions,
  typeLabels,
  typeOptions,
} from "@/features/content-audit/constants";
import {
  formatDate,
  riskVariant,
  splitCompositeId,
  statusBadge,
} from "@/features/content-audit/utils";
...
export default function ContentAuditPage() {
  ...
  const [selectedFinding, setSelectedFinding] =
    useState<PolicyAuditFinding | null>(null);
  const [aiDecision, setAiDecision] = useState<ContentAIDecision | null>(null);
  const [fixPreview, setFixPreview] = useState<ContentAIFixPreview | null>(
    null,
  );
  ...
  const canUseAIOnFinding =
    !!selectedFinding && ["article", "post"].includes(selectedFinding.type);
  ...
  const loadRuns = useCallback(async () => {
    ...
    try {
      ...
      const result = await contentAuditService.listRuns({
        page: runPage,
        per_page: 10,
      });
      ...
      setMessage({
        type: "error",
        text: error.message || "فشل تحميل سجلات الفحص",
      });
    } finally {
  ...
  const loadFindings = useCallback(async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.message || "فشل تحميل نتائج الفحص",
      });
    } finally {
  ...
  const handleStart = async () => {
    try {
      ...
      setMessage({
        type: "error",
        text: error.message || "فشل بدء فحص المحتوى",
      });
    } finally {
      setIsStarting(false);
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      setMessage({
        type: "error",
        text: error.message || "فشل تصدير تقرير الفحص",
      });
    } finally {
  ...
  const handleSelectFinding = async (finding: PolicyAuditFinding) => {
    ...
    if (!["article", "post"].includes(finding.type)) {
      setMessage({
        type: "error",
        text: "تحليل AI الحالي يدعم المقالات والمنشورات فقط لأن التصحيح يحتاج محتوى قابلًا للتعديل.",
      });
      return;
    }

    try {
      const { countryCode, id } = splitCompositeId(finding.id);
      const latest = await contentAuditService.getLatestDecision(
        finding.type,
        id,
        countryCode,
      );
  ...
  const handleAnalyzeAI = async () => {
    ...
    if (!canUseAIOnFinding) {
      setMessage({
        type: "error",
        text: "تحليل AI الحالي مخصص للمقالات والمنشورات فقط.",
      });
      return;
    }
    ...
    try {
      ...
      setMessage(null);
      const { countryCode: auditCountry, id: auditId } = splitCompositeId(
        selectedFinding.id,
      );
      ...
      setMessage({
        type: "success",
        text: "تم إنشاء قرار الذكاء الاصطناعي وحفظه بنجاح",
      });
    } catch (error: any) {
      ...
      if (status === 409) {
        text =
          "يوجد تحليل ذكاء اصطناعي قيد التنفيذ لهذا المحتوى. انتظر قليلًا ثم حدّث القرار.";
      } else if (status >= 500) {
        text =
          "فشل محرك فحص المحتوى في الخادم. تم تسجيل السبب في سجلات API، ولم يتم تعديل المحتوى.";
      } else {
        text = error.message || "فشل تحليل العنصر بالذكاء الاصطناعي";
      }
  ...
  const handleCreateFixPreview = async () => {
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: "جارٍ توليد التصحيح بواسطة AI — سيظهر المحتوى تلقائيًا عند الاكتمال.",
      });
      const preview = await contentAuditService.createAndWaitForFixPreview(
        aiDecision.id,
      );
      ...
      setMessage({
        ...
        text:
          preview.status === "failed"
            ? "فشل توليد معاينة التصحيح. راجع سجل الخادم."
            : "تم إنشاء معاينة التصحيح. راجع الفرق قبل الاعتماد.",
      });
    } catch (error: any) {
      setMessage({
        type: "error",
        text: error.message || "فشل إنشاء معاينة التصحيح",
      });
    } finally {
  ...
  const handleApplyFix = async () => {
    ...
    try {
      ...
      setFixActionError(null);
      const updated = await contentAuditService.applyFix(
        fixPreview.id,
        "تم اعتماد التصحيح من صفحة فحص المحتوى",
      );
  ...
  const handleRejectFix = async () => {
    ...
    try {
      ...
      setFixActionError(null);
      const updated = await contentAuditService.rejectFix(
        fixPreview.id,
        "تم رفض التصحيح من صفحة فحص المحتوى",
      );
      setFixPreview(updated);
    } catch (error: any) {
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardContent className="space-y-2">
            ...
            {selectedRun ? (
              <Badge
                variant={statusBadge(selectedRun.status).variant}
                dot
                animated={selectedRun.status === "running"}
              >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                        <span>
                          {run.triggered_by === "scheduled" ? "دوري" : "يدوي"}
                        </span>
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div>
                ...
                <CardDescription>
                  {selectedRun
                    ? `تقرير #${selectedRun.id} - ${formatDate(selectedRun.started_at)}`
                    : "اختر تقريرًا لعرض النتائج"}
                </CardDescription>
              </div>
              <div className="flex flex-wrap gap-2">
                {Object.entries(currentRiskCounts)
                  .slice(0, 4)
                  .map(([risk, count]) => (
          ...
          <CardContent className="space-y-4">
            <div className="grid gap-3 lg:grid-cols-[1fr_180px_180px_auto]">
              <Input name="field-app-dashboard-content-audit-page-18582-1"
                value={search}
                onChange={(event) => {
              ...
              />
              <select
                name="field-app-dashboard-content-audit-page-591-1"
                value={riskFilter}
                onChange={(event) => {
              ...
              <select
                name="field-app-dashboard-content-audit-page-605-2"
                value={typeFilter}
                onChange={(event) => {
                        ...
                        <tr
                          ...
                          className={cn(
                            ...
                            selectedFinding?.finding_id ===
                              finding.finding_id && "bg-primary/5",
                          )}
