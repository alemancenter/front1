...
import type { Message, ContactMessage, User, PaginatedResponse } from '@/types';
...
export default function MessagesPage() {
  ...
  const executeDeleteContact = async () => {
    ...
    try {
      ...
      setContactMessages(prev => prev.filter(m => m.id !== contactToDelete));
      if (selectedContactMessage?.id === contactToDelete) setSelectedContactMessage(null);
      toast.success('تم حذف الرسالة');
      ...
      <div className="grid lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-9">
          <Card className="min-h-[600px] flex flex-col">
                    ...
                    <div className="divide-y">
                      {contactMessages
                        ...
                        .map((msg) => (
                          <div
                            key={msg.id}
                            onClick={() => handleOpenContactMessage(msg)}
                            className={cn('p-4 hover:bg-muted/50 cursor-pointer transition-colors flex items-start gap-4', !msg.read ? 'bg-primary/5' : '')}
                          >
            ...
            {activeTab === 'contact' && selectedContactMessage && (
              <div className="flex flex-col h-full">
                <div className="p-4 border-b flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Button variant="ghost" size="sm" onClick={() => setSelectedContactMessage(null)}>
                      <ArrowRight className="w-4 h-4 ml-2" />
                      عودة
                    </Button>
                    <div className="h-6 w-px bg-border mx-2" />
                    <Button variant="ghost" size="icon" onClick={() => { setContactToDelete(selectedContactMessage.id); }}>
                ...
                <div className="p-6 flex-1 overflow-y-auto">
                  <div className="flex items-center gap-4 mb-6">
                    <div className="w-12 h-12 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-lg font-bold shrink-0">
                      {selectedContactMessage.name.charAt(0)}
                    </div>
                    <div>
                      <h3 className="font-semibold text-lg">{selectedContactMessage.name}</h3>
                      <p className="text-sm text-muted-foreground">{selectedContactMessage.email}</p>
                      {selectedContactMessage.phone && <p className="text-sm text-muted-foreground">{selectedContactMessage.phone}</p>}
                      {selectedContactMessage.page_url && (
                        <p className="text-xs text-muted-foreground mt-0.5 truncate max-w-xs" title={selectedContactMessage.page_url}>
                          الصفحة: {selectedContactMessage.page_url}
                        </p>
                      )}
                    </div>
                  </div>
                  <h2 className="text-xl font-bold mb-4">{selectedContactMessage.subject}</h2>
            ...
            {/* ===== REGULAR TABS (inbox / sent / drafts) ===== */}
