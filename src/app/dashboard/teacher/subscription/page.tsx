...
import { Loader2, ShieldCheck, Clock, MonitorSmartphone } from 'lucide-react';
  ...
  return (
    <div className="space-y-6" dir="rtl">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-300">إدارة حالة الاشتراك، الاستخدام، والأجهزة المرتبطة.</p>
      </div>
      ...
      {!summary?.has_active ? (
        <div className="rounded-3xl border border-amber-200 bg-amber-50 p-6">
          ...
          <p className="mt-2 text-sm text-amber-700">يمكنك طلب اشتراك الفصل الدراسي بقيمة 25 دينار أردني.</p>
      ...
      <div className="rounded-3xl bg-white p-6 shadow-sm dark:bg-slate-900">
        <h2 className="mb-4 text-lg font-black text-slate-900 dark:text-white">صلاحيات الباقة</h2>
        <div className="grid gap-3 md:grid-cols-2">
          {(summary?.plan_design?.permissions || []).map((permission) => (
            <div key={permission} className="rounded-2xl bg-slate-50 p-3 text-xs font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-200">
              {permission}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="mt-6 rounded-3xl bg-emerald-50 p-6 text-sm leading-8 text-emerald-950">
      ...
      <ol className="mt-3 list-decimal space-y-2 pr-5">
        <li>اختر المادة الأساسية التي تريد الاشتراك بها.</li>
