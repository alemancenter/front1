...
export default function CommentsPage() {
  ...
  const fetchComments = useCallback(async (page = 1) => {
    try {
      ...
      setPagination({
        ...
        per_page: meta?.per_page ?? COMMENTS_PER_PAGE,
  ...
  const selectedPendingIds = comments
    .filter((comment) => selectedIds.includes(comment.id) && comment.status !== 'approved')
    .map((comment) => comment.id);
  const canBulkApprove = selectedPendingIds.length > 0;
  ...
  const handleBulkApprove = async () => {
    ...
    try {
      ...
      await commentsService.bulkApprove(selectedDatabase, selectedPendingIds);
      setAlertConfig({
        title: 'تمت الموافقة',
        message: `تمت الموافقة على ${selectedPendingIds.length} تعليق بنجاح`,
        variant: 'success',
      });
      setAlertOpen(true);
      setSelectedIds([]);
      ...
      setAlertConfig({
        title: 'خطأ',
        message: 'فشل قبول التعليقات المحددة',
        variant: 'error',
      });
      setAlertOpen(true);
    } finally {
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6">
      ...
      {hasSelection && (
        <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <p className="text-sm font-semibold text-blue-700">
              تم تحديد {selectedIds.length} تعليق
            </p>
            <p className="text-xs text-blue-600 mt-1">
              يمكنك قبول أو حذف التعليقات المحددة من قاعدة بيانات {selectedDatabase}
            </p>
          </div>
