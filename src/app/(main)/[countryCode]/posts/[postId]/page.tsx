...
const getPost = cache(async (countryCode: string, postId: string) => {
  ...
  try {
    const response = await apiClient.get<any>(
      API_ENDPOINTS.POSTS.SHOW_PUBLIC(normalizedPostId),
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const filesCount = Array.isArray(post.files) ? post.files.length : Array.isArray(post.attachments) ? post.attachments.length : 0;
  const readiness = evaluateAdsenseReadiness({
    title: post.title,
    content: post.content,
    metaDescription: post.meta_description,
    filesCount,
    isPublished: post.is_active !== false,
  });
