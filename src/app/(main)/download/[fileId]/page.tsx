...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: item ? (type === 'post' ? `/${countryCode}/posts/${item.id}` : `/${countryCode}/lesson/articles/${item.id}`) : `/download/${fileId}`,
    },
  };
}
      ...
      <div className="container mx-auto px-4 mt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8">
            ...
            <div className="mt-8 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm font-bold leading-7 text-amber-900">
              هذه صفحة تحميل خدمية غير مخصصة للإعلانات أو الأرشفة. للحصول على السياق الكامل، يرجى الرجوع إلى الصفحة التعليمية المرتبطة بالملف.
            </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            <div className="sticky top-20 rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
              <h2 className="mb-3 text-lg font-black text-slate-950">رابط الصفحة الأصلية</h2>
              <p className="mb-4 text-sm leading-7 text-slate-600">راجع المقال أو المنشور المرتبط لمعرفة تفاصيل الملف قبل التحميل.</p>
              <Link href={backLink} className="inline-flex w-full items-center justify-center rounded-xl bg-blue-700 px-4 py-3 text-sm font-black text-white transition hover:bg-blue-800">العودة للمحتوى</Link>
            </div>
          </div>
        </div>
      </div>
